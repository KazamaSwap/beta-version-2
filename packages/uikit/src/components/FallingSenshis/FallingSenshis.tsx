...
import { BunnyProps, FallingSenshisProps } from "./types";
...
const FallingSenshis: React.FC<React.PropsWithChildren<FallingSenshisProps>> = ({
  count = 30,
  size = 32,
  iterations = Infinity,
  duration = 10,
}) => {
  const senshis = [...Array(count)].map((_, index) => (
  ...
  return <div>{senshis}</div>;
};

export default FallingSenshis;
