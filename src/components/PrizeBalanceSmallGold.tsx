...
interface PrizeBalanceSmallGoldProps extends TextProps {
  value: number
  decimals?: number
  unit?: string
  isDisabled?: boolean
...
const PrizeBalanceSmallGold: React.FC<React.PropsWithChildren<PrizeBalanceSmallGoldProps>> = ({
  value,
  color = 'text',
  decimals = 3,
  isDisabled = false,
...
export const PrizeBalanceSmallGoldWithLoading: React.FC<
  React.PropsWithChildren<Omit<PrizeBalanceSmallGoldProps, 'value'> & { value: string | number }>
> = ({ value, fontSize, ...props }) => {
  ...
  return <Header><PrizeBalanceSmallGold {...props} value={finalValue} fontSize={fontSize} /></Header>
}

export default PrizeBalanceSmallGold
