import { Flex, Text, Button, Modal, InjectedModalProps } from '@kazamaswap/uikit'
import confetti from 'canvas-confetti'
import delay from 'lodash/delay'
import { useTranslation } from '@kazamaswap/localization'
import { useEffect, useState } from 'react'
import { useSenshiSpecialXmasContract } from 'hooks/useContract'
...
const ClaimNftModal: React.FC<React.PropsWithChildren<InjectedModalProps>> = ({ onDismiss }) => {
  ...
  const senshiXmasContract = useSenshiSpecialXmasContract()
  const { toastSuccess, toastError } = useToast()

  const claimNft = async () => {
    try {
      setIsClaiming(true)
      await senshiXmasContract.mintNFT()
