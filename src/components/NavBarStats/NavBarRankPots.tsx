...
const NavBarRankPots = () => {
  const { t } = useTranslation()
  const { observerRef, isIntersecting } = useIntersectionObserver()
  const [loadData, setLoadData] = useState(false)
  const [value, setValue] = useState(0);
  const {
    data: { rankPotsBalance } = {
      rankPotsBalance: 0,
    },
  } = useSWR(
    loadData ? ['kazamaDataRow'] : null,
    async () => {
      const rankPotsKazamaCall = { address: bscTestnetTokens.kazama.address, name: 'balanceOf', params: ['0x5f230F2348EABEbEE96f93e057A40A497021c74d'], }
