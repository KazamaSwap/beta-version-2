...
const NavBarBurnGame = () => {
  const { observerRef, isIntersecting } = useIntersectionObserver()
  const [loadData, setLoadData] = useState(false)
  const [value, setValue] = useState(0);
  const {
    data: { burnGameBalance } = {
      burnGameBalance: 0,
    },
  } = useSWR(
    loadData ? ['kazamaDataRow'] : null,
    async () => {
      const burnGameUsdtCall = { address: bscTestnetTokens.usdt.address, name: 'balanceOf', params: ['0xc2478aA634618ae20a2Ec823beC69814817FFC4C'], }

      const [usdtData] = await Promise.all([
        multicallv2({
          abi: usdtAbi,
  ...
  return (
    ...
    <Flex flexDirection="row">
     ...
     <Flex flexDirection="column" ml="10px" alignContent="center">
      ...
      {burnGameBalance ? (
           <BalanceBig fontSize="15px" decimals={2} value={burnGameBalance} />
        ) : (
          <>
            <div ref={observerRef} />
