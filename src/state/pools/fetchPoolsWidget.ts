...
import poolsConfig from 'config/constants/widgetPools'
...
export const fetchPoolsBlockLimits = async () => {
  ...
  return poolsWithEnd.map((kazamaPoolConfig, index) => {
    const [[startBlock], [endBlock]] = startEndBlockResult[index]
    return {
      sousId: kazamaPoolConfig.sousId,
...
export const fetchPoolsProfileRequirement = async (): Promise<{
  ...
  const poolProfileRequireCalls = poolsWithV3
    .map((validPool) => {
      const contractAddress = getAddress(validPool.contractAddress)
      return ['pankazamaProfileIsRequested', 'pankazamaProfileThresholdPoints'].map((method) => ({
        address: contractAddress,
        name: method,
      }))
    })
