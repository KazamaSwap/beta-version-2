...
import { transformBetResponseKAZAMA, transformUserResponseKAZAMA } from './kazamaTransformers'
...
import { BetResponseKAZAMA } from './kazamaQueries'
...
export const transformBetResponse = (tokenSymbol) =>
  tokenSymbol === 'KAZAMA' ? transformBetResponseKAZAMA : transformBetResponseBNB

export const transformUserResponse = (tokenSymbol) =>
  tokenSymbol === 'KAZAMA' ? transformUserResponseKAZAMA : transformUserResponseBNB
...
export const getTotalWon = async (): Promise<{ totalWonBNB: number; totalWonKAZAMA: number }> => {
  const [{ market: BNBMarket, market: KAZAMAMarket }] = await Promise.all([
    request(
  ...
  const totalWonKAZAMA = getTotalWonMarket(KAZAMAMarket, 'KAZAMA')

  return { totalWonBNB, totalWonKAZAMA }
}
...
export const getBetHistory = async (
  ...
  api: string,
  tokenSymbol: string,
): Promise<Array<BetResponseBNB | BetResponseKAZAMA>> => {
