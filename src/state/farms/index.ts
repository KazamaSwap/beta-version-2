...
import { fetchSenshiMasterFarmPoolLength } from './fetchSenshiMasterData'
...
>(
  ...
  async ({ pids, chainId }) => {
    const [poolLength, [kazamaPerBlockRaw]] = await Promise.all([
      fetchSenshiMasterFarmPoolLength(chainId),
      multicall(senshimasterABI, [
        {
          // BSC only
          address: getSenshiMasterAddress(ChainId.BSC_TESTNET),
          name: 'kazamaPerBlock',
          params: [true],
        },
      ]),
    ...
    const regularKazamaPerBlock = getBalanceAmount(new BigNumber(kazamaPerBlockRaw))
    ...
    return [farmsWithPrices, poolLengthAsBigNumber.toNumber(), regularKazamaPerBlock.toNumber()]
  },
  {
...
>(
  ...
  async ({ account, pids, proxyAddress, chainId }, config) => {
    const poolLength = config.getState().farms.poolLength ?? (await fetchSenshiMasterFarmPoolLength(chainId))
    const farmsConfig = await getFarmConfig(chainId)
    const farmsCanFetch = farmsConfig.filter(
...
export const farmsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchFarmsPublicDataAsync.fulfilled, (state, action) => {
      const [farmPayload, poolLength, regularKazamaPerBlock] = action.payload
      ...
      state.regularKazamaPerBlock = regularKazamaPerBlock
    })
