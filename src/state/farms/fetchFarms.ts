...
import { fetchSenshiMasterData } from './fetchSenshiMasterData'
...
function farmLpTransformer(farmResult, senshiMasterResult) {
  return (farm, index) => {
    const [
    ...
    const [info, totalRegularAllocPoint] = senshiMasterResult[index]
...
const fetchFarms = async (farmsToFetch: SerializedFarmConfig[], chainId: number): Promise<SerializedFarm[]> => {
  const [farmResult, senshiMasterResult] = await Promise.all([
    fetchPublicFarmsData(farmsToFetch, chainId),
    fetchSenshiMasterData(farmsToFetch, chainId),
  ])

  return farmsToFetch.map(farmLpTransformer(farmResult, senshiMasterResult))
}
