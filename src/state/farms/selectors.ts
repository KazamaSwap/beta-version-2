...
const selectKazamaFarm = (state: State) => state.farms.data.find((f) => f.pid === 1)
...
export const priceKazamaFromPidSelector = createSelector([selectKazamaFarm], (kazamaBnbFarm) => {
  const kazamaPriceBusdAsString = kazamaBnbFarm?.tokenPriceBusd
  return new BigNumber(kazamaPriceBusdAsString)
})
...
export const farmSelector = (chainId: number) =>
  createSelector(
    ...
    (farms) => {
      ...
      const { loadArchivedFarmsData, userDataLoaded, poolLength, regularKazamaPerBlock } = farms
