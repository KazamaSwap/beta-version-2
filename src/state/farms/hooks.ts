...
import { getSenshimasterContract } from 'utils/contractHelpers'
...
export function useFarmsLength() {
  ...
  return useSWRImmutable(chainId ? ['farmsLength', chainId] : null, async () => {
    const mc = getSenshimasterContract(undefined, chainId)
 ...
 */
export const usePriceKazamaBusd = (): BigNumber => {
  const price = useKazamaBusdPrice()
