...
import { ChainId, Currency, CurrencyAmount, Pair, Trade, TradeType } from '@kazamaswap/sdk'
...
export function useDefaultsFromURLSearch():
  ...
  useEffect(() => {
    ...
    const parsed = queryParametersToSwapState(query, native.symbol, KAZAMA[chainId]?.address ?? USDC[chainId]?.address)
...
export const useLPApr = (pair?: Pair) => {
  const { data: poolData } = useSWRImmutable(
    pair && pair.chainId === ChainId.BSC_TESTNET ? ['LP7dApr', pair.liquidityToken.address] : null,
    async () => {
