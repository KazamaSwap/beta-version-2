...
import { ChainId, Pair, Token } from '@kazamaswap/sdk';

import { AppState, useAppDispatch } from '../../index';
import { GAS_PRICE_GWEI } from '../../types';
import {
    addSerializedPair, addSerializedToken, addWatchlistPool, addWatchlistToken, ChartViewMode,
    FarmStakedOnly, hidePhishingWarningBanner, muteAudio, removeSerializedToken, SerializedPair,
    setChartViewMode, setIsExchangeChartDisplayed, setSubgraphHealthIndicatorDisplayed,
    setZapDisabled, unmuteAudio, updateGasPrice, updateUserAcceptedBetaRisk, updateUserDeadline,
    updateUserExpertMode, updateUserExpertModeAcknowledgementShow, updateUserFarmStakedOnly,
    updateUserFarmsViewMode, updateUserLimitOrderAcceptedWarning, updateUserPoolStakedOnly,
    updateUserPoolsViewMode, updateUserPredictionAcceptedRisk,
    updateUserPredictionChainlinkChartDisclaimerShow, updateUserPredictionChartDisclaimerShow,
    updateUserSingleHopOnly, updateUserSlippageTolerance, updateUserUsernameVisibility, ViewMode
} from '../actions';
...
export function useZapModeManager() {
  const dispatch = useAppDispatch()
  const zapEnabled = useState(false)
...
export function useUserAcceptedBetaRisk(): [boolean, (acceptedBetaRisk: boolean) => void] {
  const dispatch = useAppDispatch()
  const userAcceptedBetaRisk = useSelector<AppState, AppState['user']['userAcceptedBetaRisk']>((state) => {
    return state.user.userAcceptedBetaRisk
  })

  const setUserAcceptedBetaRisk = useCallback(
    (acceptedBetaRisk: boolean) => {
      dispatch(updateUserAcceptedBetaRisk({ userAcceptedBetaRisk: acceptedBetaRisk }))
    },
    [dispatch],
  )

  return [userAcceptedBetaRisk, setUserAcceptedBetaRisk]
}
...
export function useGasPrice(): string {
  ...
  if (chainId === ChainId.BSC || ChainId.BSC_TESTNET) {
    return userGas
  }
