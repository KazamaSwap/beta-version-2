...
const fetchKazamaRewardsForTickets = async (
  winningTickets: LotteryTicket[],
): Promise<{ ticketsWithUnclaimedRewards: LotteryTicket[]; kazamaTotal: BigNumber }> => {
  ...
  try {
    const kazamaRewards = await multicallv2({ abi: lotteryV2Abi, calls })

    const kazamaTotal = kazamaRewards.reduce((accum: BigNumber, kazamaReward: EthersBigNumber[]) => {
      return accum.plus(new BigNumber(kazamaReward[0].toString()))
    }, BIG_ZERO)

    const ticketsWithUnclaimedRewards = winningTickets.map((winningTicket, index) => {
      return { ...winningTicket, kazamaReward: kazamaRewards[index] }
    })
    return { ticketsWithUnclaimedRewards, kazamaTotal }
  } catch (error) {
    console.error(error)
    return { ticketsWithUnclaimedRewards: null, kazamaTotal: null }
  }
}
...
export const getWinningTickets = async (
  ...
  if (unclaimedWinningTickets.length > 0) {
    const { ticketsWithUnclaimedRewards, kazamaTotal } = await fetchKazamaRewardsForTickets(unclaimedWinningTickets)
    return { ticketsWithUnclaimedRewards, allWinningTickets, kazamaTotal, roundId }
  }

  if (allWinningTickets.length > 0) {
    return { ticketsWithUnclaimedRewards: null, allWinningTickets, kazamaTotal: null, roundId }
  }
