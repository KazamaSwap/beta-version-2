...
import { kazamaSenshisAddress } from 'views/Nft/market/constants'
...
export const getCollection = async (collectionAddress: string): Promise<Record<string, Collection> | null> => {
  try {
    ...
    const totalSupplyFromApi = Number(collection?.totalSupply) || 0
    const totalSupplyFromOnChain = collectionsTotalSupply[1]
...
export const getNftsFromCollectionApi = async (
  ...
  const isKSCollection = collectionAddress.toLowerCase() === kazamaSenshisAddress.toLowerCase()
  const requestPath = `${API_NFT}/collections/${collectionAddress}/tokens${
    !isKSCollection ? `?page=${page}&size=${size}` : ``
  }`
    ...
    const res = await request(
      ...
      gql`
        query getNftCollectionMarketData($collectionAddress: String!) {
          collection(id: $collectionAddress) {
            id
            nfts(orderBy:${isKSCollection ? 'updatedAt' : 'tokenId'}, skip: $skip, first: $first) {
             ${baseNftFields}
            }
          }
        }
...
/**
 * Fetch market data for KazamaSenshis NFTs by senshi id using the Subgraph
 * @param senshiId - senshi id to query
 * @param existingTokenIds - tokens that are already loaded into redux
 * @returns
 */
export const getNftsBySenshiIdSg = async (
  senshiId: string,
  ...
  try {
    const where =
      existingTokenIds.length > 0
        ? { otherId: senshiId, isTradable: true, tokenId_not_in: existingTokenIds }
        : { otherId: senshiId, isTradable: true }
    const res = await request(
      GRAPH_API_NFTMARKET,
      gql`
        query getNftsBySenshiIdSg($collectionAddress: String!, $where: NFT_filter, $orderDirection: String!) {
      ...
      `,
      {
        collectionAddress: kazamaSenshisAddress.toLowerCase(),
        where,
        orderDirection,
      },
    )
    ...
    console.error(`Failed to fetch collection NFTs for senshi id ${senshiId}`, error)
    return []
  }
}
...
export const getAllKazamaSenshisLowestPrice = async (senshiIds: string[]): Promise<Record<string, number>> => {
  try {
    const singleKazamaSenshiSubQueries = senshiIds.map(
      (
        senshiId,
      ) => `b${senshiId}:nfts(first: 1, where: { otherId: ${senshiId}, isTradable: true }, orderBy: currentAskPrice, orderDirection: asc) {
        currentAskPrice
      }
    `,
    )
    const rawResponse: Record<string, { currentAskPrice: string }[]> = await request(
      GRAPH_API_NFTMARKET,
      gql`
        query getAllKazamaSenshisLowestPrice {
          ${singleKazamaSenshiSubQueries}
        }
      `,
    )
    return fromPairs(
      Object.keys(rawResponse).map((subQueryKey) => {
        const senshiId = subQueryKey.split('b')[1]
        return [
          senshiId,
    ...
    console.error('Failed to fetch KazamaSenshis lowest prices', error)
    return {}
  }
}

export const getAllKazamaSenshisRecentUpdatedAt = async (senshiIds: string[]): Promise<Record<string, number>> => {
  try {
    const singleKazamaSenshiSubQueries = senshiIds.map(
      (
        senshiId,
      ) => `b${senshiId}:nfts(first: 1, where: { otherId: ${senshiId}, isTradable: true }, orderBy: updatedAt, orderDirection: desc) {
        updatedAt
      }
    `,
    )
    ...
    console.error('Failed to fetch KazamaSenshis latest market updates', error)
    return {}
  }
}
...
export const getMetadataWithFallback = (apiMetadata: ApiResponseCollectionTokens['data'], senshiId: string) => {
  // The fallback is just for the testnet where some senshis don't exist
  return (
    apiMetadata[senshiId] ?? {
      name: '',
      description: '',
      collection: { name: 'Kazama Senshis' },
      image: {
        original: '',
        thumbnail: '',
      },
...
export const getKazamaSenshisAttributesField = (senshiId: string) => {
  // Generating attributes field that is not returned by API
  // but can be "faked" since objects are keyed with senshi id
  return [
    {
      traitType: 'senshiId',
      value: senshiId,
