...
export const fetchPublicPotteryValue = async (potteryVaultAddress: string) => {
  try {
    const calls = ['getStatus', 'totalLockKazama', 'totalSupply', 'lockStartTime', 'getMaxTotalDeposit'].map(
      (method) => ({
    ...
    const [getStatus, [totalLockKazama], [totalSupply], [lockStartTime], getMaxTotalDeposit] = await multicall(
      potteryVaultAbi,
      calls,
    )
    ...
    return {
      ...
      totalLockKazama: new BigNumber(totalLockKazama.toString()).toJSON(),
    ...
    return {
      ...
      totalLockKazama: BIG_ZERO.toJSON(),
...
export const fetchTotalLockedValue = async (potteryVaultAddress: string) => {
  try {
    const contract = getBep20Contract(bscTestnetTokens.kazama.address)
