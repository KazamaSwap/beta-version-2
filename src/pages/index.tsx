import { FACTORY_ADDRESS } from '@kazamaswap/sdk'
  ...
  try {
    const result = await infoServerClient.request(gql`
      query tvl {
        ...
        token(id: "0xC1498f157FC12F500a1cCd41551449A539F1ba89") {
          derivedUSD
        }
      }
    `)
    ...
    const totalKazamaInVault = await kazamaContract.balanceOf(kazamaVaultV2)
    results.tvl = parseFloat(formatEther(totalKazamaInVault)) * result.token.derivedUSD + parseFloat(totalLiquidityUSD)
  } catch (error) {
    if (process.env.NODE_ENV === 'production') {
