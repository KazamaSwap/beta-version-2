...
const VaultStakeModal: React.FC<React.PropsWithChildren<VaultStakeModalProps>> = ({
  ...
  const usdValueStaked = new BigNumber(stakeAmount).times(kazamaPriceBusd)
  const formattedUsdValueStaked = kazamaPriceBusd.gt(0) && stakeAmount ? formatNumber(usdValueStaked.toNumber()) : ''
  ...
  const handleWithdrawal = async () => {
    // trigger withdrawAll function if the withdrawal will leave 0.00001 KAZAMA or less
  ...
  const handleDeposit = async (lockDuration = 0) => {
    const receipt = await fetchWithCatchTxError(() => {
      // .toString() being called to fix a BigNumber error in prod
      // as suggested here https://github.com/ChainSafe/web3.js/issues/2077
      const extraArgs = pool.vaultKey === VaultKey.KazamaVault ? [lockDuration.toString()] : []
  ...
  if (showRoiCalculator) {
    return (
      <VaultRoiCalculatorModal
        ...
        stakingTokenBalance={kazamaAsBigNumber.plus(stakingMax)}
  ...
  return (
    ...
    >
      ...
      <BalanceInput
        ...
        currencyValue={kazamaPriceBusd.gt(0) && `~ $${formattedUsdValueStaked || 0}`}
      ...
      <Flex alignItems="center" justifyContent="space-between" mt="8px">
        <StyledButton scale="sm" mx="2px" p="4px 16px" variant="primary" onClick={() => handleChangePercent(25)}>
          25%
        </StyledButton>
        <StyledButton scale="sm" mx="2px" p="4px 16px" variant="primary" onClick={() => handleChangePercent(50)}>
          50%
        </StyledButton>
        <StyledButton scale="sm" mx="2px" p="4px 16px" variant="primary" onClick={() => handleChangePercent(75)}>
          75%
        </StyledButton>
        <StyledButton scale="sm" mx="2px" p="4px 16px" variant="primary" onClick={() => handleChangePercent(100)}>
          {t('Max')}
        </StyledButton>
      </Flex>
      ...
      {pool.vaultKey === VaultKey.KazamaVault && kazamaAsNumberBalance ? (
