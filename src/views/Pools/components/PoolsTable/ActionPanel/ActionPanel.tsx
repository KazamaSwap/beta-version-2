...
import { Box, Flex, HelpIcon, Text, useTooltip, useMatchBreakpoints } from '@kazamaswap/uikit'
import { useVaultPoolByKey } from 'state/pools/hooks'
import { getVaultPosition, VaultPosition } from 'utils/kazamaPool'
import BigNumber from 'bignumber.js'
import { DeserializedPool, VaultKey, DeserializedLockedKazamaVault, DeserializedLockedVaultUser } from 'state/types'
...
import KazamaVaultApr from './KazamaVaultApr'
...
const ActionPanel: React.FC<React.PropsWithChildren<ActionPanelProps>> = ({ account, pool, expanded }) => {
  ...
  const poolStakingTokenBalance = vaultKey
    ? kazamaAsBigNumber.plus(stakingTokenBalance)
  ...
  return (
    <StyledActionPanel expanded={expanded}>
      ...
      <ActionContainer>
        {isMobile && vaultKey === VaultKey.KazamaVault && vaultPosition === VaultPosition.None && (
          <KazamaVaultApr pool={pool} userData={vaultData.userData} vaultPosition={vaultPosition} />
        )}
        <Box width="100%">
