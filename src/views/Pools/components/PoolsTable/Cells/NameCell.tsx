import { Text, TokenPairImage as UITokenPairImage, useMatchBreakpoints } from '@kazamaswap/uikit'
...
import { getVaultPosition, VaultPosition, VaultPositionParams } from 'utils/kazamaPool'
        ...
        {showStakedTag &&
          (vaultKey === VaultKey.KazamaVault ? (
            <StakedKazamaStatus
              ...
              locked={(vaultData as DeserializedLockedKazamaVault).userData.locked}
              lockEndTime={(vaultData as DeserializedLockedKazamaVault).userData.lockEndTime}
            />
          ) : (
            <Text fontSize="12px" bold color={isFinished ? 'failure' : 'success'} textTransform="uppercase">
              {t('Staked')}
            </Text>
          ))}
...
const stakedStatus = {
  ...
  [VaultPosition.Locked]: { text: 'Locked', color: 'warning' },
...
export const StakedKazamaStatus: React.FC<React.PropsWithChildren<VaultPositionParams>> = memo(
