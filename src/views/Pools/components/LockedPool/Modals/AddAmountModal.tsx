...
import { Modal, Box, MessageText, Message, Checkbox, Flex, Text } from '@kazamaswap/uikit'
      ...
      {!checkedState && (
        <Message variant="warning" mb="16px">
          <MessageText maxWidth="320px">
            {t(
              'Adding more KAZAMA will renew your lock, setting it to remaining duration. Due to shorter lock period, benefits decrease. To keep similar benefits, extend your lock.',
            )}
          </MessageText>
        </Message>
      )}
...
const AddAmountModal: React.FC<React.PropsWithChildren<AddAmountModalProps>> = ({
  ...
  const usdValueStaked = useBUSDKazamaAmount(lockedAmountAsBigNumber.toNumber())
  const usdValueNewStaked = useBUSDKazamaAmount(totalLockedAmount)
  ...
  // if you locked for 1 week, then add KAZAMA without renew the extension, it's possible that remainingDuration + passedDuration less than 1 week.
