...
import { Flex, Text, Box, TooltipText, useTooltip, HelpIcon } from '@kazamaswap/uikit'
...
import IfoKazamaRow from './Common/IfoKazamaRow'
...
const LockedStakingApy: React.FC<React.PropsWithChildren<LockedStakingApyProps>> = ({
  ...
  const currentLockedAmountAsBigNumber = useMemo(() => {
    return userData?.balance?.kazamaAsBigNumber
  }, [userData?.balance?.kazamaAsBigNumber])

  const currentLockedAmount = getBalanceNumber(currentLockedAmountAsBigNumber)

  const usdValueStaked = useBUSDKazamaAmount(currentLockedAmount)
  ...
  const earningTokenBalance = useMemo(() => {
    return getBalanceNumber(currentLockedAmountAsBigNumber.minus(userData?.kazamaAtLastUserAction))
  }, [currentLockedAmountAsBigNumber, userData?.kazamaAtLastUserAction])
