...
import { Box, Text, Flex, MessageText, Message } from '@kazamaswap/uikit'
...
import formatiKazama from '../../utils/formatIKazama'
...
const Overview: React.FC<React.PropsWithChildren<OverviewPropsType>> = ({
  ...
  const formattediKazama = useMemo(() => {
    return formatiKazama({ lockedAmount, duration, ceiling })
  }, [lockedAmount, duration, ceiling])

  const newFormattediKazama = useMemo(() => {
    ...
    return formatiKazama({ lockedAmount: amount, duration: lockDuration, ceiling })
  ...
  return (
    <>
      <Box>
        ...
        <LightGreyCard>
          <BalanceRow title={t('KAZAMA to be locked')} value={lockedAmount} newValue={newLockedAmount} decimals={2} />
          <BalanceRow title="SENSHI" decimals={2} value={formattediKazama} newValue={newFormattediKazama} />
          <BalanceRow
            title="apy"
            unit="%"
          ...
          />
          <DateRow
            color={_toNumber(newDuration) ? 'warning' : 'text'}
            title={t('Unlock on')}
      ...
      {showLockWarning && (
        <Box mt="16px" maxWidth="370px">
          <Message variant="warning">
            <MessageText>
              {t('You will be able to withdraw the staked KAZAMA and profit only when the staking position is unlocked')}
            </MessageText>
          </Message>
        </Box>
      )}
