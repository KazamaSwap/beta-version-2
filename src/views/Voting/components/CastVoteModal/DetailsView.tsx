...
import { Flex, LinkExternal, Text, Box, HelpIcon, useTooltip, RocketIcon, Link } from '@kazamaswap/uikit'
...
const DetailsView: React.FC<React.PropsWithChildren<DetailsViewProps>> = ({
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    <>
      {Number.isFinite(lockedKazamaBalance) && (
        <Box>
          <Text>
            {isBoostingExpired
              ? t(
                  'Your vKAZAMA boosting was expired at the snapshot block. Renew your fixed-term staking position to activate the boost for future voting proposals.',
                )
              : t(
                  'Voting power is calculated using the staking amount and remaining staking duration of the fixed-term KAZAMA staking position at the block.',
                )}
          </Text>
          <Text bold m="10px 0">
            {`${t('KAZAMA locked:')} ${formatNumber(lockedKazamaBalance, 0, 2)}`}
          </Text>
  ...
  return (
    <ModalInner mb="0">
      <Text as="p" mb="24px" fontSize="14px" color="textSubtle">
        {t(
          'Your voting power is determined by the amount of KAZAMA you held and the remaining duration on the fixed-term staking position (if you have one) at the block detailed below. KAZAMA held in other places does not contribute to your voting power.',
        )}
      </Text>
      ...
      {Number.isFinite(kazamaBalance) && (
        <Flex alignItems="center" justifyContent="space-between" mb="4px">
          <Text color="textSubtle" fontSize="16px">
            {t('Wallet')}
          </Text>
          <Text textAlign="right">{formatNumber(kazamaBalance, 0, 3)}</Text>
        </Flex>
      )}
      {Number.isFinite(kazamaVaultBalance) && (
        <Flex alignItems="center" justifyContent="space-between" mb="4px">
          <Text color="textSubtle" fontSize="16px">
            {t('Flexible KAZAMA Staking')}
          </Text>
          <Text textAlign="right">{formatNumber(kazamaVaultBalance, 0, 3)}</Text>
        </Flex>
      )}
      {Number.isFinite(kazamaPoolBalance) && (
        <>
          {lockedKazamaBalance === 0 ? (
            <Flex alignItems="center" justifyContent="space-between" mb="4px">
              <Flex>
                <Text color="textSubtle" fontSize="16px">
                  {t('Fixed Term KAZAMA Staking')}
                </Text>
              ...
              <Text color="failure" textAlign="right">
                {formatNumber(kazamaPoolBalance, 0, 3)}
              </Text>
            </Flex>
          ) : (
                ...
                <Flex mt="10px" flexDirection="column" alignItems="flex-end">
                  ...
                  <Flex>
                    ...
                    <Text ml="4px" color={isBoostingExpired ? 'warning' : 'secondary'} fontSize="12px">
                      {isBoostingExpired ? t('Boosting Expired') : t('Boosted by vKAZAMA')}
                    </Text>
                  </Flex>
                </Flex>
              </FixedTermCardInner>
      ...
      {Number.isFinite(kazamaBnbLpBalance) && (
        <Flex alignItems="center" justifyContent="space-between" mb="4px">
          <Text color="textSubtle" fontSize="16px">
            {t('KAZAMA BNB LP')}
          </Text>
          <Text textAlign="right">{formatNumber(kazamaBnbLpBalance, 0, 3)}</Text>
        </Flex>
      )}
    </ModalInner>
  )
