...
const MainView: React.FC<React.PropsWithChildren<MainViewProps>> = ({
  ...
  const hasLockedKazama = lockedKazamaBalance > 0
  ...
  const hasBoosted = hasLockedKazama && !isBoostingExpired
          ...
          <>
            ...
            {total === 0 ? (
              <Message variant="danger" mb="12px">
                <Text color="danger">
                  {t(
                    'Hold some KAZAMA in your wallet or on PankazamaSwap at the snapshot block to get voting power for future proposals.',
                  )}
                </Text>
              </Message>
            ) : (
