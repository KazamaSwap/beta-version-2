...
const useNftOwner = (nft: NftToken, isOwnNft = false) => {
  ...
  const kazamaProfileAddress = getKazamaProfileAddress()
  ...
  useEffect(() => {
    const getOwner = async () => {
      try {
        if (isOwnNft && account) {
          setOwner(account)
        } else if (tokenOwner && tokenOwner.toLowerCase() !== kazamaProfileAddress.toLowerCase()) {
          setOwner(tokenOwner)
        } else {
          setOwner(null)
        }
  ...
  }, [account, isOwnNft, currentSeller, collectionContract, tokenId, tokenOwner, kazamaProfileAddress])
