...
import { Skeleton, Table, Td, Th, Flex, ArrowUpIcon, ArrowDownIcon } from '@kazamaswap/uikit'
...
import { useGetLowestPriceFromSenshiId } from '../../hooks/useGetLowestPrice'
...
interface KazamaSenshisTraitsProps {
  collectionAddress: string
}

const LowestPriceCell: React.FC<React.PropsWithChildren<{ senshiId: string }>> = ({ senshiId }) => {
  const { isFetching, lowestPrice } = useGetLowestPriceFromSenshiId(senshiId)
...
const KazamaSenshisTraits: React.FC<React.PropsWithChildren<KazamaSenshisTraitsProps>> = ({ collectionAddress }) => {
  ...
  return (
    <>
      {!isFetchingDistribution ? (
        <CollapsibleCard title={t('Senshi Id')}>
          <TableWrapper>
            <Table>
              <thead>
                <tr>
              ...
              <tbody>
                {sortedTokenList.map((token) => {
                  ...
                  const percentage = (count / totalSenshiCount) * 100
                  const handleClick = () => {
                  ...
                  return (
                    <ClickableRow key={token.tokenId} onClick={handleClick} title={t('Click to view NFT')}>
                      ...
                      <Td textAlign="right" width="100px">
                        <LowestPriceCell senshiId={token.tokenId} />
                      </Td>
                    </ClickableRow>
                  )
                })}
...
export default KazamaSenshisTraits
