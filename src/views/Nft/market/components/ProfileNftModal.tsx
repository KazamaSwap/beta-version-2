import { InjectedModalProps, Modal, Flex, Text, Button, useModal, Link, Grid, LinkExternal } from '@kazamaswap/uikit'
...
const ProfileNftModal: React.FC<React.PropsWithChildren<ProfileNftModalProps>> = ({ nft, onDismiss, onSuccess }) => {
  ...
  const itemPageUrlId = nft.collectionAddress === kazamaSenshisAddress ? nft.attributes[0].value : nft.tokenId
    ...
    <StyledModal title={t('Details')} onDismiss={onDismiss} headerBackground={theme.colors.gradients.cardHeader}>
      <Flex flexDirection="column" maxWidth="420px">
        ...
        <TextWrapper p="24px 16px" flexDirection="column">
          <Text mb="16px">{t("You're using this NFT as your Kazama Profile picture")}</Text>
