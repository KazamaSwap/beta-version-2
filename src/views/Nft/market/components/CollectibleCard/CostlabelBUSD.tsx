...
import { Currency, Price } from '@kazamaswap/sdk'
...
interface CostLabelBUSDProps extends FlexProps {
  cost: number
  bnbBusdPrice: Price<Currency, Currency>
}

export const CostLabelBUSD: React.FC<React.PropsWithChildren<CostLabelBUSDProps>> = ({ cost, bnbBusdPrice, ...props }) => {
...
export const CostLabelBNB: React.FC<React.PropsWithChildren<CostLabelBUSDProps>> = ({ cost, bnbBusdPrice, ...props }) => {
  const priceInUsd = multiplyPriceByAmount(bnbBusdPrice, cost)
    ...
    <Flex alignItems="center" {...props}>
       {priceInUsd > 0 && (
        <Text fontSize="14px" bold color="textSubtle">
          Buy for {`$${priceInUsd.toLocaleString(undefined, {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        })}`}</Text>
      )}
...
export const MetaRow: React.FC<React.PropsWithChildren<MetaRowProps>> = ({ title, children, ...props }) => (
  <Flex alignItems="center" justifyContent="space-between" {...props}>
    <Text fontSize="14px" color="textSubtle" maxWidth="120px" ellipsis title={title}>
      {title}
    </Text>
    <Box>{children}</Box>
  </Flex>
  ...
  return (
    <MetaRow title={t('Lowest price')}>
      {isFetching ? (
        ...
        <CostLabelBUSD cost={lowestPrice} bnbBusdPrice={bnbBusdPrice} />
      )}
    </MetaRow>
  )
}
