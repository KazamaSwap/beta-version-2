...
import { Modal, Text, Flex, BalanceInput, Box, Button, LogoRoundIcon } from '@kazamaswap/uikit'
...
import { useKazama, useFarmAuctionContract } from 'hooks/useContract'
...
const PlaceBidModal: React.FC<React.PropsWithChildren<PlaceBidModalProps>> = ({
  ...
  useEffect(() => {
    ...
    if (fetchStatus === FetchStatus.Fetched && userKazamaBalance.lt(bid)) {
  ...
  }, [isMultipleOfTen, isMoreThanInitialBidAmount, userNotEnoughKazama, initialBidAmount, t, isFirstBid])

  const { isApproving, isApproved, isConfirmed, isConfirming, handleApprove, handleConfirm } =
    useApproveConfirmTransaction({
      onRequiresApproval: async () => {
        return requiresApproval(kazamaContractReader, account, farmAuctionContract.address)
      },
      onApprove: () => {
        return callWithGasPrice(kazamaContractApprover, 'approve', [farmAuctionContract.address, MaxUint256])
      },
