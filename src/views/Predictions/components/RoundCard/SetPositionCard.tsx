...
import { useGetBnbBalance, useGetKazamaBalance } from 'hooks/useTokenBalance'
...
import useKazamaApprovalStatus from 'hooks/useKazamaApprovalStatus'
import useKazamaApprove from 'hooks/useKazamaApprove'
...
const SetPositionCard: React.FC<React.PropsWithChildren<SetPositionCardProps>> = ({
  ...
  const { isVaultApproved, setLastUpdated } = useKazamaApprovalStatus(token.symbol === 'KAZAMA' ? predictionsAddress : null)
  const { handleApprove, pendingTx } = useKazamaApprove(
    setLastUpdated,
    predictionsAddress,
  ...
  const doesKazamaApprovePrediction = token.symbol === 'BNB' || isVaultApproved
  ...
  const handleEnterPosition = async () => {
    ...
    const args = token.symbol === 'KAZAMA' ? [epoch, valueAsBn.toString()] : [epoch]
