import { Box, CoinSwitcher, Flex, PocketWatchIcon, Text, CloseIcon } from '@kazamaswap/uikit'
...
export const PricePairLabel: React.FC<React.PropsWithChildren> = () => {
  ...
  const onTokenSwitch = useCallback(() => {
    if (router.query.token === PredictionSupportedSymbol.KAZAMA) {
      router.query.token = PredictionSupportedSymbol.BNB
    } else if (router.query.token === undefined && token.symbol === PredictionSupportedSymbol.KAZAMA) {
      router.query.token = PredictionSupportedSymbol.BNB
