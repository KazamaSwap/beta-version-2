...
import { Text, Flex, Box, Radio, Skeleton, LinkExternal, Link, ArrowForwardIcon, ArrowBackIcon, IconButton} from '@kazamaswap/uikit'
...
import { PageButtons } from 'views/Nft/market/components/PaginationButtons'
...
const ResponsiveGrid = styled.div`
  ...
  grid-gap: 1em;
  align-items: center;
  grid-template-columns: 1.3fr 1.75fr repeat(4, 1fr);
  padding: 0 24px;
...
const PaginationContainer = styled(PageButtons)`
width: 100%;
display: flex;
align-items: center;
...
const DataRow: React.FC<React.PropsWithChildren<{ transaction: Transaction }>> = ({ transaction }) => {
  ...
  return (
    <ResponsiveGrid>
           <ProfileCell accountAddress={transaction.sender} />
      <Link href={getBlockExploreLink(transaction.hash, 'transaction')}>
        <Text fontSize="15px">
          {transaction.type === TransactionType.MINT
            ? t('Add %token0% / %token1%', { token0: transaction.token0Symbol, token1: transaction.token1Symbol })
            : transaction.type === TransactionType.SWAP
            ? t('Swap %token0% for %token1%', { token0: inputTokenSymbol, token1: outputTokenSymbol })
            : t('Remove %token0% / %token1%', { token0: transaction.token0Symbol, token1: transaction.token1Symbol })}
        </Text>
      </Link>
      <Text fontSize="15px">${formatAmount(transaction.amountUSD)}</Text>
      <Text fontSize="15px">
       <Text fontSize="15px">{`${formatAmount(abs0)} ${transaction.token0Symbol}`}</Text>
      </Text>
      <Text fontSize="15px">
        <Text fontSize="15px">{`${formatAmount(abs1)} ${transaction.token1Symbol}`}</Text>
      </Text>
      {/* <LinkExternal href={getBlockExploreLink(transaction.sender, 'address')}>
        {truncateHash(transaction.sender)}
      </LinkExternal> */}
      <Text fontSize="15px">{formatDistanceToNowStrict(parseInt(transaction.timestamp, 10) * 1000)}</Text>
    </ResponsiveGrid>
  )
}
    ...
    <Wrapper>
      <Flex mb="8px">
        ...
        <Flex ml="auto">
              ...
              >
        <StyledIconButton color={page === 1 ? 'textDisabled' : 'primary'} mr="10px">
          <ArrowBackIcon />
        </StyledIconButton>
              </Arrow>
              <Arrow
              ...
              >
        <StyledIconButton color={page === maxPage ? 'textDisabled' : 'primary'}>
          <ArrowForwardIcon />
        </StyledIconButton>
              </Arrow>
            </Flex>
      ...
      <TableWrapper>
        ...
        <ClickableColumnHeader
          ...
          >
            {t('User Account')} {arrow(SORT_FIELD.sender)}
          </ClickableColumnHeader>
          ...
          <>
            ...
            {sortedTransactions.length === 0 ? (
              <Flex justifyContent="center">
                <Text fontSize="15px">{t('No Transactions')}</Text>
              </Flex>
            ) : undefined}
          </>
        ) : (
