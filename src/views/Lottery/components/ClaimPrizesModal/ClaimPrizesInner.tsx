...
import { Flex, Button, Text, AutoRenewIcon, PresentWonIcon } from '@kazamaswap/uikit'
...
const ClaimInnerContainer: React.FC<React.PropsWithChildren<ClaimInnerProps>> = ({ onSuccess, roundsToClaim }) => {
  ...
  const kazamaReward = activeClaimData.kazamaTotal
  const dollarReward = kazamaReward.times(kazamaPriceBusd)
  const rewardAsBalance = getBalanceAmount(kazamaReward).toNumber()
    ...
    if (receipt?.status) {
      toastSuccess(
        ...
        <ToastDescriptionWithTx txHash={receipt.transactionHash}>
          {t('Your KAZAMA prizes for round %lotteryId% have been sent to your wallet', { lotteryId })}
