...
import { useKazama, useLotteryV2Contract } from 'hooks/useContract'
...
import EditNumbersBox from './EditNumbersBox'
...
const BuyTicketsBox: React.FC<React.PropsWithChildren<BuyTicketsBoxProps>> = ({ onDismiss }) => {
  ...
  const { balance: userKazama, fetchStatus } = useTokenBalance(bscTestnetTokens.kazama.address)
  ...
  const stringifiedUserKazama = userKazama.toJSON()
  const memoisedUserKazama = useMemo(() => new BigNumber(stringifiedUserKazama), [stringifiedUserKazama])

  const kazamaPriceBusd = usePriceKazamaBusd()
  const dispatch = useAppDispatch()
  const hasFetchedBalance = fetchStatus === FetchStatus.Fetched
  const userKazamaDisplayBalance = getFullDisplayBalance(userKazama, 18, 3)
  ...
  const validateInput = useCallback(
    ...
    [limitNumberByMaxTicketsPerBuy, getTicketCostAfterDiscount, maxNumberTicketsPerBuyOrClaim, userKazama],
  )

  useEffect(() => {
    const getMaxPossiblePurchase = () => {
      const maxBalancePurchase = memoisedUserKazama.div(priceTicketInKazama)
      const limitedMaxPurchase = limitNumberByMaxTicketsPerBuy(maxBalancePurchase)
      let maxPurchase

      // If the users' max KAZAMA balance purchase is less than the contract limit - factor the discount logic into the max number of tickets they can purchase
  ...
  useEffect(() => {
    ...
    const costBeforeDiscount = priceTicketInKazama.times(numberOfTicketsToBuy)
  ...
  }, [ticketsToBuy, priceTicketInKazama, discountDivisor, getTicketCostAfterDiscount])
  ...
  const { isApproving, isApproved, isConfirmed, isConfirming, handleApprove, handleConfirm } =
    useApproveConfirmTransaction({
      onRequiresApproval: async () => {
        return requiresApproval(kazamaContractReader, account, lotteryContract.address)
      },
      onApprove: () => {
        return callWithGasPrice(kazamaContractApprover, 'approve', [lotteryContract.address, MaxUint256])
      },
  ...
  const getErrorMessage = () => {
    if (userNotEnoughKazama) return t('Insufficient KAZAMA balance')
  ...
  return (
    <StyledModal>
      ...
      <BalanceInput
        isWarning={account && (userNotEnoughKazama || maxTicketPurchaseExceeded)}
        placeholder="0"
        value={ticketsToBuy}
        onUserInput={handleInputChange}
        currencyValue={
          kazamaPriceBusd.gt(0) &&
          `~${ticketsToBuy ? getFullDisplayBalance(priceTicketInKazama.times(new BigNumber(ticketsToBuy))) : '0.00'} KAZAMA`
        }
      />
      <Flex alignItems="center" justifyContent="flex-end" mt="4px" mb="12px">
        <Flex justifyContent="flex-end" flexDirection="column">
          {account && (userNotEnoughKazama || maxTicketPurchaseExceeded) && (
      ...
      <Flex flexDirection="column">
        <Flex mb="8px" justifyContent="space-between">
          ...
          <Text color="textSubtle" fontSize="14px">
            {priceTicketInKazama && getFullDisplayBalance(priceTicketInKazama.times(ticketsToBuy || 0))} KAZAMA
          </Text>
        </Flex>
        <Flex mb="8px" justifyContent="space-between">
          <Flex>
