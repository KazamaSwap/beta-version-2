...
import { Text, Flex, AccountIcon, TeamBattleIcon, Box, useTooltip, LinkExternal } from '@kazamaswap/uikit'
...
const NotOkNFT = ({ admissionProfile }) => {
  ...
  const keyword = '%Kazama Squad NFT%'

  const rawText = t(`Set %Kazama Squad NFT% as Kazama Profile avatar`)
      ...
      element={
        <>
          <LinkExternal
            ...
            href={`https://kazamaswap.finance/nfts/collections/${admissionProfile}`}
          >
            {t('Kazama Squad NFT')}
          </LinkExternal>
          <br />
...
const NotOkProfilePoints = ({ pointThreshold }) => {
  ...
  const keyword = '%Kazama Profile%'

  const rawText = t(`Reach %point% or more %Kazama Profile% points`, { point: pointThreshold })
    ...
    <TransWithElement
      ...
      element={
        <>
          ...
          <LinkExternal
            style={{ display: 'inline' }}
            href={`https://kazamaswap.finance/profile/${account}/achievements/`}
          >
            {t('Kazama Profile')}
          </LinkExternal>
        </>
