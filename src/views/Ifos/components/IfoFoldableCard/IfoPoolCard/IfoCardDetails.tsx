...
import { Text, Flex, Box, Skeleton, TooltipText, useTooltip } from '@kazamaswap/uikit'
...
const MaxTokenEntry = ({ maxToken, ifo, poolId }: { maxToken: number; ifo: Ifo; poolId: PoolIds }) => {
  const isCurrencyKazama = ifo.currency === bscTestnetTokens.kazama
  ...
  const basicTooltipContent =
    ifo.version >= 3.1
      ? t(
          'For the private sale, each eligible participant will be able to commit any amount of KAZAMA up to the maximum commit limit, which is published along with the IFO voting proposal.',
        )
      : t(
          'For the basic sale, Max KAZAMA entry is capped by minimum between your average KAZAMA balance in the iKAZAMA, or the pool’s hard cap. To increase the max entry, Stake more KAZAMA into the iKAZAMA',
        )

  const unlimitedToolipContent =
    ifo.version >= 3.1 ? (
      <Box>
        <Text display="inline">{t('For the public sale, Max KAZAMA entry is capped by')} </Text>
        <Text bold display="inline">
          {t('the number of iKAZAMA.')}{' '}
        </Text>
        <Text display="inline">
          {t('Lock more KAZAMA for longer durations to increase the maximum number of KAZAMA you can commit to the sale.')}
        </Text>
      </Box>
    ) : (
      t(
        'For the unlimited sale, Max KAZAMA entry is capped by your average KAZAMA balance in the iKazama. To increase the max entry, Stake more KAZAMA into the iKazama',
      )
    )
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(tooltipContent, { placement: 'bottom-start' })
  const label = isCurrencyKazama ? t('Max. KAZAMA entry') : t('Max. token entry')
        ...
        value={
          <Text small textAlign="right" color={maxToken > 0 ? 'text' : 'failure'}>
            {`${formatNumber(maxToken, 3, 3)} ${
              !isCurrencyKazama ? ifo.currency.symbol : ''
    ...
    if (status === 'coming_soon') {
      return (
        <>
          ...
          {ifo[poolId].kazamaToBurn !== '$0' && <FooterEntry label={t('KAZAMA to burn:')} value={ifo[poolId].kazamaToBurn} />}
          <FooterEntry
