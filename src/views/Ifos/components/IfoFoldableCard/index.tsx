...
const StyledNoHatSenshi = styled.div<{ $isLive: boolean; $isCurrent?: boolean }>`
  position: absolute;
  left: -24px;
  z-index: 1;
  top: 33px;
...
const NoHatSenshi = ({ isLive, isCurrent }: { isLive?: boolean; isCurrent?: boolean }) => {
  const { isXs, isSm, isMd } = useMatchBreakpoints()
  const isSmallerThanTablet = isXs || isSm || isMd
  if (isSmallerThanTablet && isLive) return null
  return (
    <StyledNoHatSenshi $isLive={isLive} $isCurrent={isCurrent}>
      <img
        src={`/images/ifos/assets/senshipop-${!isSmallerThanTablet ? 'right' : 'left'}.png`}
        width={123}
        height={162}
        alt="senshi"
      />
...
export const IfoCurrentCard = ({
  ...
  const shouldShowSenshi = publicIfoData.status === 'live' || publicIfoData.status === 'coming_soon'
    ...
    <>
      {isMobile && (
        ...
        >
          ...
          {shouldShowSenshi && <NoHatSenshi isLive={publicIfoData.status === 'live'} />}
        </Box>
      )}
      <Box position="relative" width="100%" maxWidth={['400px', '400px', '400px', '400px', '400px', '100%']}>
        {!isMobile && shouldShowSenshi && <NoHatSenshi isCurrent isLive={publicIfoData.status === 'live'} />}
...
const IfoFoldableCard = ({
  ...
  return (
    <Box id={ifo.id} ref={wrapperEl} position="relative">
      {isExpanded && isDesktop && <NoHatSenshi isLive={false} />}
