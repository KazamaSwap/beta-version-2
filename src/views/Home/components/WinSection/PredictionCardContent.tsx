...
import { ArrowForwardIcon, Button, Flex, Heading, Skeleton, Text } from '@kazamaswap/uikit'
import { NextLinkFromReactRouter } from 'components/NextLink'
import { useTranslation } from '@kazamaswap/localization'
import { formatLocalisedCompactNumber } from 'utils/formatBalance'
import useIntersectionObserver from 'hooks/useIntersectionObserver'
import { getTotalWon } from 'state/predictions/helpers'
import { useBNBBusdPrice, useKazamaBusdPrice } from 'hooks/useBUSDPrice'
...
const PredictionCardContent = () => {
  ...
  const kazamaWonInUsd = multiplyPriceByAmount(kazamaPriceBusd, data?.totalWonKAZAMA || 0)

  const localisedBnbUsdString = formatLocalisedCompactNumber(bnbWonInUsd + kazamaWonInUsd)
  const bnbWonText = t('$%wonInUsd% in BNB + KAZAMA won so far', { wonInUsd: localisedBnbUsdString })
    ...
    <>
      <Flex flexDirection="column" mt="48px">
        ...
        <Text color="#280D5F" mb="40px">
          {t('Predict the price trend of BNB or KAZAMA to win')}
        </Text>
      </Flex>
