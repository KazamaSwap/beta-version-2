import { appearAnimation, useIsomorphicEffect, useMatchBreakpoints, Text } from '@kazamaswap/uikit'
...
const BannerPlaceHolder = styled.div<{ walletConnected: boolean }>`
  ...
  &::before {
    ...
    right: 0px;
    bottom: 0px;
    background: url(/images/SenshiBanner.jpg) no-repeat center center fixed; 
  ...
  ${({ theme }) => theme.mediaQueries.lg},${({ theme }) => theme.mediaQueries.md} {
    padding-top: 0;
    margin-top: ${({ walletConnected }) => (walletConnected ? '-30px' : '-32px')};
    margin-bottom: ${({ walletConnected }) => (walletConnected ? '5px' : '30px')};
  }
`
