...
const initialData = {
  data: {
    sousId: 0,
    stakingToken: serializeToken(bscTestnetTokens.kazama),
    earningToken: serializeToken(bscTestnetTokens.kazama),
    contractAddress: {
      ...
      const fetchPoolsUserDataAsync = async () => {
        try {
          const [stakedBalances, pendingRewards, totalStaking] = await Promise.all([
            ...
            kazamaContract.balanceOf(initialData.data.contractAddress[chainId]),
          ])
