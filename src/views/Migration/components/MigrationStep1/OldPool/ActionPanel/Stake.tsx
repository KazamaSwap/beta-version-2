...
const Staked: React.FC<React.PropsWithChildren<StackedActionProps>> = ({ pool }) => {
  ...
  let kazamaAsBigNumber = new BigNumber(0)
  let kazamaAsNumberBalance = 0
  if (pricePerFullShare) {
    const { kazamaAsBigNumber: kazamaBigBumber, kazamaAsNumberBalance: kazamaBalance } = convertSharesToKazama(
      userShares,
      pricePerFullShare,
    )
    kazamaAsBigNumber = kazamaBigBumber
    kazamaAsNumberBalance = kazamaBalance
  }

  const stakedAutoDollarValue = getBalanceNumber(kazamaAsBigNumber.multipliedBy(stakingTokenPrice), stakingToken.decimals)

  const balance = vaultKey ? (Number.isNaN(kazamaAsNumberBalance) ? 0 : kazamaAsNumberBalance) : stakedTokenBalance
