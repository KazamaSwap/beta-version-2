import { Box, Flex, Text, useMatchBreakpoints } from '@kazamaswap/uikit'
...
const StakedCell: React.FC<React.PropsWithChildren<StakedCellProps>> = ({ pool }) => {
  ...
  if (pricePerFullShare) {
    const { kazamaAsNumberBalance: kazamaBalance } = convertSharesToKazama(userShares, pricePerFullShare)
        ...
        <Flex>
          <Box mr="8px" height="32px">
            <Balance
              ...
              value={pool.vaultKey ? (Number.isNaN(kazamaAsNumberBalance) ? 0 : kazamaAsNumberBalance) : stakedTokenBalance}
            />
          </Box>
        </Flex>
      </CellContent>
