...
import { Heading, Text, Button, ArrowForwardIcon, Link } from '@kazamaswap/uikit'
...
const MigrationPage: React.FC<React.PropsWithChildren> = () => {
  ...
  const steps: Step[] = [
    {
      ...
      canHover: true,
      text: t('Unstake LP tokens and KAZAMA from the old SenshiMaster contract.'),
    },
    {
      stepId: ProgressStepsType.STEP2,
      canHover: true,
      text: t('Stake LP tokens and KAZAMA to the new SenshiMaster v2 contract.'),
    },
  ]
  ...
  const { data: kazamaPool, userDataLoaded } = useFetchUserPools(account)

  const v1Pools = useMemo(() => {
    const ifoPoolVault = { ...kazamaPool, vaultKey: VaultKey.IfoPool }
    const kazamaAutoVault = { ...kazamaPool, vaultKey: VaultKey.KazamaVaultV1 }

    return [ifoPoolVault, kazamaAutoVault, kazamaPool]
  ...
  return (
    <div ref={tableWrapperEl}>
      <PageHeader>
        ...
        <Link href="https://docs.pankazamaswap.finance/code/migration/migrate-your-stakings" external>
