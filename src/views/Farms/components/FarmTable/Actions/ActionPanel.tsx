...
import { LinkExternal, Text, useMatchBreakpoints, ActionPanelLink } from '@kazamaswap/uikit'
...
const ActionPanel: React.FunctionComponent<React.PropsWithChildren<ActionPanelProps>> = ({
  ...
  const lpLabel = farm.lpSymbol && farm.lpSymbol.toUpperCase().replace('KAZAMA', 'KAZAMA')
  ...
  return (
    <Container expanded={expanded}>
      <InfoContainer>
        ...
        {isActive && (
          <StakeContainer>
            <ActionPanelLink href={`/add/${liquidityUrlPathParts}`} bold={false} small>
              {t('Get %symbol%', { symbol: lpLabel })}
            </ActionPanelLink>
          </StakeContainer>
        )}
        <ActionPanelLink href={bsc} bold={false} small>{t('View Contract')}</ActionPanelLink>
        <ActionPanelLink href={info} bold={false} small>{t('See Pair Info')}</ActionPanelLink>
      </InfoContainer>
      <ActionContainer>
        {shouldUseProxyFarm ? (
