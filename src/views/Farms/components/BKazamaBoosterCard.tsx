...
import { useBKazamaProxyContractAddress } from '../hooks/useBKazamaProxyContractAddress'
import { useUserBoosterStatus } from '../hooks/useUserBoosterStatus'
import { useUserLockedKazamaStatus } from '../hooks/useUserLockedKazamaStatus'
...
export const BKazamaBoosterCard = () => {
  ...
  const tooltipContent = (
    <>
      <Box mb="20px">
        {t(
          'Yield Boosters allow you to boost your farming yields by locking KAZAMA in the fixed-term staking KAZAMA pool. The more KAZAMA you lock, and the longer you lock them, the higher the boost you will receive.',
        )}
      </Box>
      <Box>
        {t('To learn more, check out the')}
        <Link target="_blank" href="https://kazamaswap.finance">
          {t('Medium Article')}
        </Link>
      </Box>
    </>
...
const CardContent: React.FC = () => {
  ...
  const { proxyCreated, refreshProxyAddress } = useBKazamaProxyContractAddress(account)
    ...
    return (
      <Box>
        ...
        <Text color="textSubtle" fontSize={12} mb="16px">
          {t('An active fixed-term KAZAMA staking position is required for activating farm yield boosters.')}
        </Text>
