...
export const StepperWrapper = styled.div<{ finished: boolean; active: boolean }>`
  ...
  &:not(:last-child) {
    ...
    &::before {
      ...
      background-color: ${({ theme, finished }) => (finished ? theme.colors.success : theme.colors.disabled)};
    }
  ...
  ${StepperCircle} {
    background: ${({ theme, finished }) => (finished ? theme.colors.success : theme.colors.disabled)};
  }
  ${StepperText} {
...
export const InfoIconBox = styled.div`
  ...
  align-items: center;
`
interface BKazamaMigrateModalProps {
  lpContract: Contract
...
export const BKazamaMigrateModal: React.FC<BKazamaMigrateModalProps> = ({
  lpContract,
  stakedBalance,
  onDismiss,
  onUnStack,
  ...
  useEffect(() => {
    if (!bKazamaProxy) return
    bKazamaProxy.lpApproved(lpContract.address).then((enabled) => {
      setIsApproved(enabled)
    })
  }, [lpContract, bKazamaProxy])
              ...
              {step === activatedState.toString() ? (
                ...
                <StepperCircle>
                  {parseInt(step) < parseInt(activatedState.toString()) ? (
                    <CheckmarkIcon color="text" width={18} />
                  ) : (
                    index + 1
                  )}
                </StepperCircle>
