...
import Settings from 'components/Menu/GlobalSettings/Settings';
...
import { CurrencyAmount, JSBI, MINIMUM_LIQUIDITY, Token, WNATIVE } from '@kazamaswap/sdk';
import { KAZAMA, USDC } from '@kazamaswap/tokens';
import {
    AddIcon, Button, ButtonMenu, ButtonMenuItem, CardBody, Flex, GasIcon, Heading, IconButton, Link,
    Message, MessageText, Text, TooltipText, useModal, useTooltip
} from '@kazamaswap/uikit';

import { AppBodyLiq, AppHeader } from '../../components/App';
...
import LiqNav from '../../components/Menu/LiqNav';
...
import {
    useDerivedMintInfo, useMintActionHandlers, useMintState, useZapIn
} from '../../state/mint/hooks';
import { useTransactionAdder } from '../../state/transactions/hooks';
import {
    useGasPrice, useGasPriceManager, useIsExpertMode, usePairAdder, useUserSlippageTolerance,
    useZapModeManager
...
const IconWrapper = styled(SwitchIconButton)`
border-radius: 50%;
background: #191e2e;
z-index: 100;
width: 42px;
...
const InfoCard = styled(LightCard)`
...
export default function AddLiquidity() {
  ...
  const [currencyIdA, currencyIdB] = router.query.currency || [
    native.symbol,
    KAZAMA[chainId]?.address ?? USDC[chainId]?.address,
  ]
  ...
  const showAddLiquidity =
    (!!currencies[Field.CURRENCY_A] && !!currencies[Field.CURRENCY_B]) || !canZap
          ...
          {showAddLiquidity && (
            <>
                 ...
                 <div style={{display: "flex", alignItems: "center"}}>
                  ...
                  <div style={{display: "flex", alignItems: "center"}}>
                  <Settings mode={SettingsMode.SWAP_LIQUIDITY} />
                  </div>
                 </div>
                 </div>
              <Flex padding="15px">
                <AutoColumn gap="20px">
                  ...
                  {currencies[Field.CURRENCY_A] && currencies[Field.CURRENCY_B] && pairState !== PairState.INVALID && (
                    <>
                        <InfoCard padding="1rem" borderRadius="20px">
                          <PoolPriceBar
                            currencies={currencies}
