...
const useTokenBalance = (tokenAddress: string, forceBSC?: boolean) => {
  ...
  const key = useMemo(
    () =>
      account
        ? {
            contract: forceBSC ? contract.connect(bscTestRpcProvider) : contract,
...
export const useGetBnbBalance = () => {
  ...
  const { status, data, mutate } = useSWR([account, 'bnbBalance'], async () => {
    return bscTestRpcProvider.getBalance(account)
  })

  return { balance: data || Zero, fetchStatus: status, refresh: mutate }
}

export const useGetKazamaBalance = () => {
  const { chainId } = useWeb3React()
  const { balance, fetchStatus } = useTokenBalance(KAZAMA[chainId]?.address || KAZAMA[ChainId.BSC_TESTNET]?.address, true)
