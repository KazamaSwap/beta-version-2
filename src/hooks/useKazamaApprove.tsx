...
const useKazamaApprove = (setLastUpdated: () => void, spender, successMsg) => {
  ...
  const handleApprove = async () => {
    const receipt = await fetchWithCatchTxError(() => {
      return callWithGasPrice(kazamaContract, 'approve', [spender, MaxUint256])
    })
    if (receipt?.status) {
      toastSuccess(
        t('Contract Enabled'),
...
export default useKazamaApprove
