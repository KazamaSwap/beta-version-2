...
import { getLanguageCodeFromLS } from '@kazamaswap/localization'
...
export const getFullDisplayBalance = (balance: BigNumber, decimals = 18, displayDecimals = 2): string => {
...
export const formatNumberExact = (number: number, minPrecision = 0, maxPrecision = 0) => {
  const options = {
    minimumFractionDigits: minPrecision,
    maximumFractionDigits: maxPrecision,
  }
...
export const formatLpBalance = (balance: BigNumber) => {
  ...
  return stakedBalanceBigNumber.toFixed(2, BigNumber.ROUND_DOWN)
}
